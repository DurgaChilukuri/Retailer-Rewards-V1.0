...
public class CustomerRewardsCalculatorSvcImpl implements CustomerRewardsCalculatorSvc {
	...
	public Integer getEarnedRewards(String customerId) {
		...
		customersTransactionData.stream()
	...
	private Integer findUnrewardedAmountSpentInAQuarter(List<TransactionDetails> customerTransactionDetails) {
		...
		List<TransactionDetails> rewardsEligibleTransactionList = customerTransactionDetails.stream()
				.filter(transaction -> !transaction.getIsTransactionAlreadyRewarded()).filter(transaction -> (Duration
						.between(LocalDate.of(2022, 5, 15).atStartOfDay(), transaction.getTransactionDate()).toDays() < 90))
				.collect(Collectors.toList());

		Double totalSpentAmount = rewardsEligibleTransactionList.stream().map(TransactionDetails::getTotalSpentAmount).reduce(0.00,
				(sum, elt) -> sum + elt);
		...
		Integer accruedRewards = rewardPointsForSpentAmountGreaterThan50.intValue() + rewardPointsForSpentAmountGreaterThan100.intValue();
	...
	private List<Customer> getMockData() {
		...
		/**
		 * one transaction lesser than 50 and one transaction greater than 100
		 */
		...
		/**
		 * some transactions are already rewarded and
		 * some transactions are less than $50 and not rewarded
		 */
		...
		/**
		 * one transaction is less than 50
		 * one transaction greater than $50 but less than $100
		 */
